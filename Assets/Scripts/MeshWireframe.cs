    ...
    private string UpdateMesh(int triangleNumberChange)
    {
        ...
        if (triangleNumberChange > 0 && newTrianglesAmount > _originalTriangles.Length)
        {
            if (triangleNumberChange == 1)
                return FormErrorMessage("Cannot add more triangles - all of them in place already");
            return FormErrorMessage("Not enough space left to add " + triangleNumberChange + " triangles");
        }
